...
## PCA
cwines <- wines[,1:11]
prwine <- prcomp(cwines)
summary(prwine)
round(prwine$rot[,1],2)

## When scaled PC1 proportion of variance drops from 95.38% to 27.54%. Total.sulfur.dioxide is the primary determinant, with most variables having no impact
prwinec <- prcomp(cwines, scale=TRUE)
summary(prwinec)

# Looking at first primary component, acidity, chlorides, density, pH, sulphates and alcohol have a negative effect.
round(prwinec$rot[,1],2)
# For the second primary component, only pH and alcohol have a negative effect, with the others switching signs.
round(prwinec$rot[,2],2)
# Looking at third primary component, pH becomes strongly positive, but alcohol remains negative. Citric acid becomes strongly negative. Sugar and sulfur dioxide (free and total) are positive for each of the top three.
round(prwinec$rot[,3],2)

#These results run counter to the regression model, as alcohol and pH both have positive coefficients. 

#Eigenvectors for the PCA
matplot(1:11, prwinec$rot[,1:3], type="l", xlab="Factors", ylab
        ="", col=1)
